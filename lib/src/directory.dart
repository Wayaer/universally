...
class DirectoryUtil {
    ...
    if (_singleton == null) {
      await _lock.synchronized(() async {
        if (_singleton == null) {
          ...
          // 保持本地实例直到完全初始化。
          final DirectoryUtil singleton = DirectoryUtil._();
  ...
  Future<void> _init() async {
    ...
    print(
        'DirectoryUtil init : ${DateTime.now().millisecondsSinceEpoch - old}');
  }
  ...
  static Future<Directory?> initStorageDir() async {
    if (_storageDir == null && Platform.isAndroid)
  ...
  static Future<Directory?> createDir(String? path) async {
    ...
    if (!exist) dir = await dir.create(recursive: true);
    return dir;
  }
  ...
  static String? getPath(
    ...
    final StringBuffer sb = StringBuffer(dir.path);
  ...
  /// Android: /data/user/0/{包名}/cache/Pictures/demo.png
  /// iOS: xxx;
  static String? getTempPath({
    ...
    String? format,
      ...
      getPath(_tempDir, category: category, fileName: fileName, format: format);
  ...
  /// Android: /data/user/0/{包名}/app_flutter/Pictures/demo.png
  /// iOS: xxx;
  ...
  /// Android: /data/user/0/{包名}/files/Pictures/demo.png
  /// iOS: xxx;
  ...
  /// Android: /storage/emulated/0/Android/data/{包名}/files/Download/demo.apk
  /// iOS: xxx;
  ...
  static Directory? createTempDirSync({String? category}) {
    final String? path = getTempPath(category: category);
    return createDirSync(path);
  }

  static Directory? createAppDocDirSync({String? category}) {
    final String? path = getAppDocPath(category: category);
    return createDirSync(path);
  }

  static Directory? createAppSupportDirSync({String? category}) {
    final String? path = getAppSupportPath(category: category);
    return createDirSync(path);
  }

  static Directory? createStorageDirSync({String? category}) {
    final String? path = getStoragePath(category: category);
    return createDirSync(path);
  }
