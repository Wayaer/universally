...
def show_help():
    print(f"{GREEN}===== Flutter Windows打包脚本使用说明 ====={NC}")
    print(f"{GREEN}用法:{NC} {sys.argv[0]} [选项]")
    print()
    print(f"{GREEN}选项:{NC}")
    print(f"{GREEN}  -n app_name      应用名称{NC} (默认: 空)")
    print(f"{GREEN}  -c channel       渠道名{NC} (默认: windows)")
...
def validate_parameters(args):
    ...
    if not os.path.isdir("windows"):
        print(f"{RED}错误: 未找到windows目录，请确认当前目录是Flutter项目根目录且已配置windows平台{NC}", file=sys.stderr)
        print(f"{YELLOW}提示: 可以使用 'flutter create --platforms windows .' 命令添加windows平台支持{NC}", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser.add_argument("-c", dest="channel", default="windows", help="渠道名")
 ...
 {YELLOW}└---------------------------------------------------------------{NC}""")
    ...
    # 执行打包命令
    build_command = f"flutter build windows --{args.build_type} --analyze-size {dart_define} -t {args.main_path}"
    ...
    # 准备输出目录
    output_dir = f"app/windows/{args.build_type}/"
    os.makedirs(output_dir, exist_ok=True)
    print(f"{BLUE}输出目录: {output_dir}{NC}")

    # 打包输出
    src = f"build/windows/x64/runner/{args.build_type}"
    dest = f"{output_dir}{args.app_name}-{args.channel}-v{version}-{current_date}"
    ...
    if os.path.isdir(src):
        ...
        if os.path.exists(dest):
            import shutil
            shutil.rmtree(dest)
            print(f"{YELLOW}警告: 目标目录已存在，已删除旧版本{NC}")
